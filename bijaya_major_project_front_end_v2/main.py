...
#         new_disease = Plant(name=name, image_path=image_path)
...
@app.route('/water_plant/<int:plant_id>', methods=['POST', 'GET'])
def water_plant(plant_id):
    if request.method == 'POST':
        print(f'\n\n Watering plant with id {plant_id} \n\n')
...
def add_disease():
    if request.method == 'POST':
        ...
        # image = request.files['image']
        # image_path = upload_image(image)
        new_disease = Plant(name=name, plant_id=json.dumps(plant_id))# , image_path=image_path)
        session.add(new_disease)
        session.commit()
        return redirect(url_for('display_diseases'))
...
def delete_disease(disease_id):
    if request.method == 'POST':
        disease = session.query(Plant).get(disease_id)
        if disease:
...
def display_diseases():
    all_diseases = session.query(Plant).all()
